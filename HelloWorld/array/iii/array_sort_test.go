...
func Test_Array_Sort(T *testing.T){
	array:=[]int{7,0,1,4,8}
	//quick_srot(array,0,len(array)-1)
...
func quick_srot(array []int,start,end int){
	low,high,key:=start,end,array[start]
	...
	if low-1>=start{
		quick_srot(array,start,low-1)
	}

	if high+1<end{
		quick_srot(array,high+1,end)
	}
}